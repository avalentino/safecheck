...
MANIFEST_SCHEMA_STR = """<?xml version="1.0"?>
...
XmlSchemaType = Union[str, os.PathLike]
...
def md5sum(filename: os.PathLike) -> str:
    ...
    with open(filename, 'rb') as fd:
        for data in iter(functools.partial(fd.read, 65536), b""):
            md5.update(data)
...
def verify_safe_product(product: os.PathLike,
    ...
    for filename in sorted(files):
        logger.warning(f"file '{filename}' found in product but not included in manifest.safe")
        has_warnings = True
...
def main(*argv):
    ...
    args, unused_args = version_parser.parse_known_args(argv if argv else None)
    if args.version:
    ...
    try:
        ...
        logger.debug('Keyboard interrupt received: exit the program')
        return 1
